...
static int ctp_connect(void *sock_hdl, const struct sockaddr *name, socklen_t namelen)
{
...
void *ctp_cli_reg(u16_t id, struct sockaddr_in *dest_addr, int (*cb_func)(void *hdl, enum ctp_cli_msg_type type, const char *topic, const char *parm_list, void *priv), void *priv)
{
    ...
    if (ctp_connect(hdl->sock_hdl, (struct sockaddr *)&hdl->dest_addr, sizeof(struct sockaddr)) == -1) {
