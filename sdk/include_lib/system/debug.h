#ifndef _LIB_SYSTEM_DEBUG_H_
#define _LIB_SYSTEM_DEBUG_H_
...
{ \
    if(config_ulog_enable){ \
        if (LOG_IS_ENABLE(LOG_VERBOSE)){ \
            dbg_print("[Verb]: " _LOG_TAG format "\r\n", ## __VA_ARGS__); \
...
{ \
    if(config_ulog_enable){ \
        if (LOG_IS_ENABLE(LOG_INFO)){ \
            dbg_print("[Info]: " _LOG_TAG format "\r\n", ## __VA_ARGS__); \
...
{ \
    if(config_ulog_enable){ \
        if (LOG_IS_ENABLE(LOG_DEBUG)) { \
            dbg_print("[Debug]: " _LOG_TAG format "\r\n", ## __VA_ARGS__); \
...
{ \
    if(config_ulog_enable) { \
        if (LOG_IS_ENABLE(LOG_ERROR)) { \
            dbg_print("[Error]: " _LOG_TAG format "\r\n", ## __VA_ARGS__); \
...
{ \
    if(config_ulog_enable) { \
        if (LOG_IS_ENABLE(LOG_WARN)) { \
            dbg_print("[Warn]: " _LOG_TAG format "\r\n", ## __VA_ARGS__); \
