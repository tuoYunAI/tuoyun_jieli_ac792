...
#pragma bss_seg(".jl_kws_main.data.bss")
#pragma data_seg(".jl_kws_main.data")
#pragma const_seg(".jl_kws_main.text.const")
#pragma code_seg(".jl_kws_main.text")
#endif
...
int jl_kws_speech_recognition_start(void)
{
    kws_info("%s", __func__);
    if (__this->task_init) {
        os_taskq_post_msg(THIS_TASK_NAME, 1, KWS_SPEECH_RECOGNITION_RUN);
    }
...
void jl_kws_speech_recognition_stop(void)
{
    ...
    if (__this->task_init) {
        ...
        os_taskq_post_msg(THIS_TASK_NAME, 1, KWS_SPEECH_RECOGNITION_STOP);
    }
}
...
void jl_kws_speech_recognition_close(void)
{
    ...
    if (__this->task_init) {
        ...
        /*使用里面的os_sem_post,防止jl_kws_audio_get_data一直pend timeout重新打开mic*/
        kws_aec_data_output(NULL, NULL, 0);
        os_sem_create(&del_sem, 0);
        for (int i = 0; i < 10; i++) {
            err = os_taskq_post_msg(THIS_TASK_NAME, 2, KWS_SPEECH_RECOGNITION_CLOSE, (void *)&del_sem);
            if (err != OS_Q_FULL) {
                break;
            }
            os_time_dly(2);
            printf("jl kws task qfull\n");
        }
        if (err == OS_NO_ERR) {
