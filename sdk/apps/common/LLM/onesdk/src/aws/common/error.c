...
{
//    printf("Need to make sure \"aws_raise_error_private\" runs OK!");
    printf("%s err:%d", __FUNCTION__, err);
//     tl_last_error = err;
...
static aws_error_handler_fn *s_global_handler = NULL;
static void *s_global_error_context = NULL;
static aws_error_handler_fn *s_thread_handler = NULL;
static void *s_thread_handler_context = NULL;
...
static const struct aws_error_info *get_error_by_code(int err)
{
    if (err >= (AWS_ERROR_ENUM_STRIDE * AWS_PACKAGE_SLOTS) || err < 0) {
        return NULL;
    }

    uint32_t slot_index = (uint32_t)err >> AWS_ERROR_ENUM_STRIDE_BITS;
    uint32_t error_index = (uint32_t)err & (AWS_ERROR_ENUM_STRIDE - 1);
...
{
    const struct aws_error_info *error_info = get_error_by_code(err);
    return error_info ? error_info->error_str : "Unknown Error Code";
}

const char *aws_error_name(int err)
{
    const struct aws_error_info *error_info = get_error_by_code(err);
    return error_info ? error_info->literal_name : "Unknown Error Code";
}

const char *aws_error_lib_name(int err)
{
    const struct aws_error_info *error_info = get_error_by_code(err);
    return error_info ? error_info->lib_name : "Unknown Error Code";
}

const char *aws_error_debug_str(int err)
{
    const struct aws_error_info *error_info = get_error_by_code(err);
    return error_info ? error_info->formatted_name : "Unknown Error Code";
}
...
void aws_raise_error_private(int err)
{
    ...
    /* 先调用线程级handler，再调用全局handler */
    if (s_thread_handler) {
        s_thread_handler(err, s_thread_handler_context);
    } else if (s_global_handler) {
        s_global_handler(err, s_global_error_context);
    }
}
...
void aws_reset_error(void)
{
    g_last_error = AWS_ERROR_SUCCESS;
}
...
aws_error_handler_fn *aws_set_global_error_handler_fn(aws_error_handler_fn *handler, void *ctx)
{
    aws_error_handler_fn *old = s_global_handler;
    s_global_handler = handler;
    s_global_error_context = ctx;
    return old;
}
...
aws_error_handler_fn *aws_set_thread_local_error_handler_fn(aws_error_handler_fn *handler, void *ctx)
{
    aws_error_handler_fn *old = s_thread_handler;
...
void aws_register_error_info(const struct aws_error_info_list *error_info)
{
    if (!error_info || !error_info->error_list || error_info->count == 0) {
        printf("Invalid error info list\n");
        abort();
    }

    const int min_code = error_info->error_list[0].error_code;
    const int slot_index = min_code >> AWS_ERROR_ENUM_STRIDE_BITS;

    if (slot_index < 0 || slot_index >= AWS_PACKAGE_SLOTS) {
        printf("Error code %d out of slot range\n", min_code);
        abort();
    }

    /* 检查错误码连续性 */
#ifdef DEBUG
    for (size_t i = 0; i < error_info->count; ++i) {
        if (error_info->error_list[i].error_code != min_code + (int)i) {
            fprintf(stderr, "Error codes must be consecutive starting from %d\n", min_code);
            abort();
        }
    }
#endif
...
int aws_translate_and_raise_io_error_or(int error_no, int fallback)
{
    switch (error_no) {
    case EINVAL:
        /* If useful fallback code provided, raise that instead of AWS_ERROR_INVALID_ARGUMENT,
...
//                return aws_raise_error(AWS_ERROR_INVALID_ARGUMENT);
//            }
        return aws_raise_error(fallback != AWS_ERROR_SYS_CALL_FAILURE ?
                               fallback : AWS_ERROR_INVALID_ARGUMENT);
    case EPERM:
    case EACCES:
        return aws_raise_error(AWS_ERROR_NO_PERMISSION);
    case EISDIR:
    ...
    case ENOTEMPTY:
        return aws_raise_error(AWS_ERROR_DIRECTORY_NOT_EMPTY);
    default:
        return aws_raise_error(fallback);
    }
}
