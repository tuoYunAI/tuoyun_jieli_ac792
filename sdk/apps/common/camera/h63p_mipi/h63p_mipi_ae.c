#include "h63p_mipi_ae.h"
#include "h63p_mipi.h"

#include "asm/isp_alg.h"

extern u32 H63P_MIPI_calc_shutter(isp_ae_shutter_t *shutter, u32 exp_time_us, u32 gain);
extern u32 H63P_MIPI_set_shutter(isp_ae_shutter_t *shutter);
extern void  *H63P_MIPI_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev);



//Anti-flicker:50Hz
const static u32 H63P_MIPI_ae_50hz_25fps[][AE_CURVE_INFO_MAX] = {
...
const static u32 H63P_MIPI_ae_50hz_30fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
    {9102000,     30,       100,    AE_INTERP_EXP},  // lv = 20
    {27306,    10000,       100,    AE_INTERP_GAIN}, // lv = 10.74
    {13653,    20000,       100,    AE_INTERP_GAIN},
...
const static u32 H63P_MIPI_ae_60hz_30fps[][AE_CURVE_INFO_MAX] = {
...
const static u32 H63P_MIPI_ae_50hz_var_fps[][AE_CURVE_INFO_MAX] = {
    // EV       ExpoTime    ISO     INTERPOLATE
...
static isp_ae_params_t  H63P_MIPI_ae_params = {
    ...
    .ae_ops = {
        .calc_shutter = H63P_MIPI_calc_shutter,
        .set_shutter = H63P_MIPI_set_shutter,
        .get_ae_curve = H63P_MIPI_get_ae_curve,
    },
};


void H63P_MIPI_ae_ev_init(u32 fps)
{
    u32 time, gain;
    int minev, maxev;
    ...
    ae_curve = H63P_MIPI_get_ae_curve(AE_CURVE_TYPE, fps, &minev, &maxev);
#ifdef ISP0_EN
    isp_ae_curve_interp(ae_curve, AE_INIT_EV, &time, &gain);
#endif

    H63P_MIPI_calc_shutter(&ae_shutter, time, gain);

    H63P_MIPI_set_shutter(&ae_shutter);
}

void  *H63P_MIPI_get_ae_curve(u32 type, u32 fps, int *min_ev, int *max_ev)
{
    void *curve;

    *min_ev = AE_MIN_EV;
    *max_ev = AE_MAX_EV;
    curve = &H63P_MIPI_ae_50hz_25fps;
    ...
    if (type == AE_CURVE_50HZ) {
        ...
        *max_ev = AE_MAX_EV;
        ...
        if (fps == 25) {
            curve = &H63P_MIPI_ae_50hz_25fps;
            printf("\n----ppk 25fps H63P_MIPI ppk-----\n");
            printf("csi_tval_con = %04x\n", csi_tval_con);
            printf("csi_phy_con2 = %08x\n", (u32 *)(csi_phy_con2));
            if (csi_tval_con) {

                //csi_phy_con2->x0_hs_en = 1;
                /* wrH63P_MIPIReg(0x12, 0x00); */
            }


        } else if (fps == 30) {
            curve = &H63P_MIPI_ae_50hz_30fps;
            printf("\n----ppk 30fps H63P_MIPI ppk-----\n");
        ...
        *max_ev = AE_MAX_EV;
        ...
        curve = &H63P_MIPI_ae_60hz_30fps;
        printf("\n----ppk 30fps 60hz H63P_MIPI ppk-----\n");
...
void *H63P_MIPI_get_ae_params()
{
    return (void *)&H63P_MIPI_ae_params;
}
