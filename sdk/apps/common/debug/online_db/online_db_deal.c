...
static void db_data_try_send(void)
{
    ...
    if (send_len > DB_TMP_PACKET_LEN) {
        ASSERT(send_len <= DB_TMP_PACKET_LEN);
    }
...
static struct db_online_api_t de_online_api_table = {
    .init = db_init,
    .exit = db_exit,
...
static const u8 test_buffer_data[32] = {1, 2, 3, 4, 5, 6, 7, 8, 9};
