...
void mijia_uECC_set_rng(uECC_RNG_Function rng_function)
{
...
uECC_VLI_API void mijia_uECC_vli_nativeToBytes(uint8_t *bytes,
        int num_bytes,
...
uECC_VLI_API void mijia_uECC_vli_bytesToNative(uint8_t *native,
        const uint8_t *bytes,
        int num_bytes)
{
    mijia_uECC_vli_nativeToBytes(native, num_bytes, bytes);
}

#else

void mijia_uECC_vli_nativeToBytes(uint8_t *bytes,
                                  int num_bytes,
...
void mijia_uECC_vli_bytesToNative(uECC_word_t *native,
                                  const uint8_t *bytes,
                                  int num_bytes)
{
    wordcount_t i;
...
int mijia_uECC_make_key(uint8_t *public_key,
                        uint8_t *private_key,
                        uECC_Curve curve)
{
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    ...
    for (tries = 0; tries < uECC_RNG_MAX_TRIES; ++tries) {
        ...
        if (EccPoint_compute_public_key(_public, _private, curve)) {
            ...
            mijia_uECC_vli_nativeToBytes(private_key, BITS_TO_BYTES(curve->num_n_bits), _private);
            mijia_uECC_vli_nativeToBytes(public_key, curve->num_bytes, _public);
            mijia_uECC_vli_nativeToBytes(
...
int mijia_uECC_shared_secret(const uint8_t *public_key,
...
#else
    mijia_uECC_vli_bytesToNative(_private, private_key, BITS_TO_BYTES(curve->num_n_bits));
    mijia_uECC_vli_bytesToNative(_public, public_key, num_bytes);
    mijia_uECC_vli_bytesToNative(_public + num_words, public_key + num_bytes, num_bytes);
#endif
...
#else
    mijia_uECC_vli_nativeToBytes(secret, num_bytes, _public);
#endif
    return !EccPoint_isZero(_public, curve);
}

#if uECC_SUPPORT_COMPRESSED_POINT
void mijia_uECC_compress(const uint8_t *public_key, uint8_t *compressed, uECC_Curve curve)
{
    wordcount_t i;
...
void mijia_uECC_decompress(const uint8_t *compressed, uint8_t *public_key, uECC_Curve curve)
{
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
...
#else
    mijia_uECC_vli_bytesToNative(point, compressed + 1, curve->num_bytes);
#endif
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN == 0
    mijia_uECC_vli_nativeToBytes(public_key, curve->num_bytes, point);
    mijia_uECC_vli_nativeToBytes(public_key + curve->num_bytes, curve->num_bytes, y);
#endif
}
#endif /* uECC_SUPPORT_COMPRESSED_POINT */

int mijia_uECC_valid_point(const uECC_word_t *point, uECC_Curve curve)
{
    uECC_word_t tmp1[uECC_MAX_WORDS];
    uECC_word_t tmp2[uECC_MAX_WORDS];
    wordcount_t num_words = curve->num_words;
...
int mijia_uECC_valid_public_key(const uint8_t *public_key, uECC_Curve curve)
{
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    uECC_word_t *_public = (uECC_word_t *)public_key;
#else
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN == 0
    mijia_uECC_vli_bytesToNative(_public, public_key, curve->num_bytes);
    mijia_uECC_vli_bytesToNative(
        _public + curve->num_words, public_key + curve->num_bytes, curve->num_bytes);
#endif
    return mijia_uECC_valid_point(_public, curve);
}

int mijia_uECC_compute_public_key(const uint8_t *private_key, uint8_t *public_key, uECC_Curve curve)
{
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    uECC_word_t *_private = (uECC_word_t *)private_key;
    uECC_word_t *_public = (uECC_word_t *)public_key;
...
#else
    mijia_uECC_vli_bytesToNative(native, bits, bits_size);
#endif
...
static int mijia_uECC_sign_with_k(const uint8_t *private_key,
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN == 0
    mijia_uECC_vli_nativeToBytes(signature, curve->num_bytes, p); /* store r */
#endif

#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    bcopy((uint8_t *) tmp, private_key, BITS_TO_BYTES(curve->num_n_bits));
#else
    mijia_uECC_vli_bytesToNative(tmp, private_key, BITS_TO_BYTES(curve->num_n_bits)); /* tmp = d */
#endif
...
#else
    mijia_uECC_vli_nativeToBytes(signature + curve->num_bytes, curve->num_bytes, s);
#endif
    return 1;
}

int mijia_uECC_sign(const uint8_t *private_key,
                    const uint8_t *message_hash,
                    unsigned hash_size,
                    uint8_t *signature,
                    uECC_Curve curve)
{
    ...
    for (tries = 0; tries < uECC_RNG_MAX_TRIES; ++tries) {
        ...
        if (mijia_uECC_sign_with_k(private_key, message_hash, hash_size, k, signature, curve)) {
            return 1;
        }
    }
    return 0;
...
int mijia_uECC_sign_deterministic(const uint8_t *private_key,
                                  const uint8_t *message_hash,
                                  unsigned hash_size,
...
filled:
        ...
        if (mijia_uECC_sign_with_k(private_key, message_hash, hash_size, T, signature, curve)) {
            return 1;
        }
...
int mijia_uECC_verify(const uint8_t *public_key,
                      const uint8_t *message_hash,
                      unsigned hash_size,
                      ...
                      uECC_Curve curve)
{
    ...
    mijia_uECC_vli_bytesToNative(r, signature, curve->num_bytes);
    mijia_uECC_vli_bytesToNative(s, signature + curve->num_bytes, curve->num_bytes);
#endif
