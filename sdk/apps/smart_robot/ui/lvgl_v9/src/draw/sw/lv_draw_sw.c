...
void lv_draw_sw_init(void)
{
    ...
    for (i = 0; i < LV_DRAW_SW_DRAW_UNIT_CNT; i++) {
        ...
        if (i == 0) {
            lv_thread_init(&thread_dsc->thread, "lv_swdraw_unit_0", LV_DRAW_THREAD_PRIO, render_thread_cb,
                           LV_DRAW_THREAD_STACK_SIZE, thread_dsc);
        } else if (i == 1) {
            lv_thread_init(&thread_dsc->thread, "lv_swdraw_unit_1", LV_DRAW_THREAD_PRIO, render_thread_cb,
                           LV_DRAW_THREAD_STACK_SIZE, thread_dsc);
        } else {
            assert(0);
        }
