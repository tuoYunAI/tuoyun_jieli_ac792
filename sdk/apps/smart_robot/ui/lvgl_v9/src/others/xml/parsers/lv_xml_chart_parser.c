...
static lv_chart_update_mode_t chart_update_mode_to_enum(const char *txt)
{
    if (lv_streq("shift", txt)) {
        return LV_CHART_UPDATE_MODE_SHIFT;
    }
    if (lv_streq("circular", txt)) {
...
static lv_chart_axis_t chart_axis_to_enum(const char *txt)
{
    if (lv_streq("primary_x", txt)) {
        return LV_CHART_AXIS_PRIMARY_X;
    }
    if (lv_streq("primary_y", txt)) {
        return LV_CHART_AXIS_PRIMARY_Y;
    }
    if (lv_streq("secondary_x", txt)) {
        return LV_CHART_AXIS_SECONDARY_X;
    }
    if (lv_streq("secondary_y", txt)) {
