...
void lv_xml_table_apply(lv_xml_parser_state_t *state, const char **attrs)
{
    ...
    for (int i = 0; attrs[i]; i += 2) {
        ...
        if (lv_streq("column_count", name)) {
            lv_table_set_column_count(item, lv_xml_atoi(value));
        } else if (lv_streq("row_count", name)) {
            lv_table_set_row_count(item, lv_xml_atoi(value));
...
void lv_xml_table_column_apply(lv_xml_parser_state_t *state, const char **attrs)
{
    ...
    for (int i = 0; attrs[i]; i += 2) {
        ...
        if (lv_streq("width", name)) {
            lv_table_set_column_width(table, column, lv_xml_atoi(value));
        }
    }
}
...
static lv_table_cell_ctrl_t table_ctrl_to_enum(const char *txt)
{
    if (lv_streq("none", txt)) {
        return LV_TABLE_CELL_CTRL_NONE;
    }
    if (lv_streq("merge_right", txt)) {
        return LV_TABLE_CELL_CTRL_MERGE_RIGHT;
    }
    if (lv_streq("text_crop", txt)) {
        return LV_TABLE_CELL_CTRL_TEXT_CROP;
    }
    if (lv_streq("custom_1", txt)) {
        return LV_TABLE_CELL_CTRL_CUSTOM_1;
    }
    if (lv_streq("custom_2", txt)) {
        return LV_TABLE_CELL_CTRL_CUSTOM_2;
    }
    if (lv_streq("custom_3", txt)) {
        return LV_TABLE_CELL_CTRL_CUSTOM_3;
    }
    if (lv_streq("custom_4", txt)) {
        return LV_TABLE_CELL_CTRL_CUSTOM_4;
    }
