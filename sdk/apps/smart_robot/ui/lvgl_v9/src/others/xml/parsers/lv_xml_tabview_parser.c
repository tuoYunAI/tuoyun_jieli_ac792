...
void lv_xml_tabview_apply(lv_xml_parser_state_t *state, const char **attrs)
{
    ...
    for (int i = 0; attrs[i]; i += 2) {
        ...
        if (lv_streq("active", name)) {
            lv_tabview_set_active(item, lv_xml_atoi(value), 0);
        }
        if (lv_streq("tab_bar_position", name)) {
            lv_tabview_set_tab_bar_position(item, lv_xml_dir_to_enum(value));
        }
    }
}
