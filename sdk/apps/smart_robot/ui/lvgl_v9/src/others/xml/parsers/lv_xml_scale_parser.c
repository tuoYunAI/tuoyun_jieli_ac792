...
void lv_xml_scale_apply(lv_xml_parser_state_t *state, const char **attrs)
{
    ...
    for (int i = 0; attrs[i]; i += 2) {
        ...
        if (lv_streq("mode", name)) {
            lv_scale_set_mode(item, scale_mode_to_enum(value));
        } else if (lv_streq("total_tick_count", name)) {
            lv_scale_set_total_tick_count(item, lv_xml_atoi(value));
        } else if (lv_streq("major_tick_every", name)) {
            lv_scale_set_major_tick_every(item, lv_xml_atoi(value));
        } else if (lv_streq("label_show", name)) {
            lv_scale_set_label_show(item, lv_xml_to_bool(value));
        } else if (lv_streq("post_draw", name)) {
            lv_scale_set_post_draw(item, lv_xml_to_bool(value));
        } else if (lv_streq("draw_ticks_on_top", name)) {
            lv_scale_set_draw_ticks_on_top(item, lv_xml_to_bool(value));
        ...
        } else if (lv_streq("angle_range", name)) {
            lv_scale_set_angle_range(item, lv_xml_to_bool(value));
        } else if (lv_streq("rotation", name)) {
            lv_scale_set_rotation(item, lv_xml_to_bool(value));
        }
    }
}
...
static lv_scale_mode_t scale_mode_to_enum(const char *txt)
{
    if (lv_streq("horizontal_top", txt)) {
        return LV_SCALE_MODE_HORIZONTAL_TOP;
    }
    if (lv_streq("horizontal_bottom", txt)) {
        return LV_SCALE_MODE_HORIZONTAL_BOTTOM;
    }
    if (lv_streq("vertical_left", txt)) {
        return LV_SCALE_MODE_VERTICAL_LEFT;
    }
    if (lv_streq("vertical_right", txt)) {
        return LV_SCALE_MODE_VERTICAL_RIGHT;
    }
    if (lv_streq("round_inner", txt)) {
        return LV_SCALE_MODE_ROUND_INNER;
    }
    if (lv_streq("round_outer", txt)) {
        return LV_SCALE_MODE_ROUND_OUTER;
    }
