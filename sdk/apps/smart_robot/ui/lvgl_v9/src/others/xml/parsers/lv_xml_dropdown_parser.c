...
void lv_xml_dropdown_apply(lv_xml_parser_state_t *state, const char **attrs)
{
    ...
    for (int i = 0; attrs[i]; i += 2) {
        ...
        if (lv_streq("options", name)) {
            lv_dropdown_set_options(item, value);
        } else if (lv_streq("text", name)) {
            lv_dropdown_set_text(item, value);
        } else if (lv_streq("selected", name)) {
            lv_dropdown_set_selected(item, lv_xml_atoi(value));
        } else if (lv_streq("symbol", name)) {
            lv_dropdown_set_symbol(item, lv_xml_get_image(&state->scope, value));
