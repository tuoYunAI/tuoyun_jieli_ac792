...
static lv_bar_orientation_t orentation_text_to_enum_value(const char *txt)
{
    ...
    if (lv_streq("horizontal", txt)) {
        return LV_BAR_ORIENTATION_HORIZONTAL;
    }
    if (lv_streq("vertical", txt)) {
        return LV_BAR_ORIENTATION_VERTICAL;
    }
...
static lv_bar_mode_t mode_text_to_enum_value(const char *txt)
{
    ...
    if (lv_streq("range", txt)) {
        return LV_BAR_MODE_RANGE;
    }
    if (lv_streq("symmetrical", txt)) {
        return LV_BAR_MODE_SYMMETRICAL;
    }
