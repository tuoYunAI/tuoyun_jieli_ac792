...
void lv_xml_spangroup_apply(lv_xml_parser_state_t *state, const char **attrs)
{
    ...
    for (int i = 0; attrs[i]; i += 2) {
        ...
        if (lv_streq("overflow", name)) {
            lv_spangroup_set_overflow(item, spangroup_overflow_to_enum(value));
        } else if (lv_streq("max_lines", name)) {
            lv_spangroup_set_max_lines(item, lv_xml_atoi(value));
        } else if (lv_streq("indent", name)) {
            lv_spangroup_set_indent(item, lv_xml_atoi(value));
        }
    }
}
...
void lv_xml_spangroup_span_apply(lv_xml_parser_state_t *state, const char **attrs)
{
    ...
    for (int i = 0; attrs[i]; i += 2) {
        ...
        if (lv_streq("text", name)) {
            lv_spangroup_set_span_text(spangroup, span, value);
...
static lv_span_overflow_t spangroup_overflow_to_enum(const char *txt)
{
    if (lv_streq("clip", txt)) {
        return LV_SPAN_OVERFLOW_CLIP;
    }
    if (lv_streq("ellipsis", txt)) {
        return LV_SPAN_OVERFLOW_ELLIPSIS;
    }
