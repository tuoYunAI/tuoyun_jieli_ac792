...
static lv_keyboard_mode_t mode_text_to_enum_value(const char *txt)
{
    if (lv_streq("text_upper", txt)) {
        return LV_KEYBOARD_MODE_TEXT_UPPER;
    }
    if (lv_streq("text_lower", txt)) {
        return LV_KEYBOARD_MODE_TEXT_LOWER;
    }
#if LV_USE_ARABIC_PERSIAN_CHARS == 1
    if (lv_streq("text_arabic", txt)) {
        return LV_KEYBOARD_MODE_TEXT_ARABIC;
    }
#endif
    if (lv_streq("number", txt)) {
        return LV_KEYBOARD_MODE_NUMBER;
    }
    if (lv_streq("special", txt)) {
