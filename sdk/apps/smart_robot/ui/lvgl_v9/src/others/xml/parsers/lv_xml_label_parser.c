...
void lv_xml_label_apply(lv_xml_parser_state_t *state, const char **attrs)
{
    ...
    for (int i = 0; attrs[i]; i += 2) {
        ...
        if (lv_streq("long_mode", name)) {
            lv_label_set_long_mode(item, long_mode_text_to_enum_value(value));
        }
...
static lv_label_long_mode_t long_mode_text_to_enum_value(const char *txt)
{
    if (lv_streq("wrap", txt)) {
        return LV_LABEL_LONG_MODE_WRAP;
    }
    if (lv_streq("scroll", txt)) {
        return LV_LABEL_LONG_MODE_SCROLL;
    }
    if (lv_streq("scroll_circular", txt)) {
        return LV_LABEL_LONG_MODE_SCROLL_CIRCULAR;
    }
    if (lv_streq("dots", txt)) {
        return LV_LABEL_LONG_MODE_DOTS;
    }
    if (lv_streq("clip", txt)) {
        return LV_LABEL_LONG_MODE_CLIP;
    }
