...
void lv_obj_set_name(lv_obj_t *obj, const char *name)
{
    ...
    if (!obj->spec_attr->name_static && obj->spec_attr->name) {
...
lv_obj_t *lv_obj_get_child_by_name(const lv_obj_t *parent, const char *path)
{
    ...
    if (parent == NULL || parent->spec_attr == NULL || path == NULL) {
        return NULL;
    }
...
{
    if (parent == NULL) {
        parent = lv_display_get_screen_active(NULL);
    }
    if (parent == NULL) {
        return NULL;
    }
