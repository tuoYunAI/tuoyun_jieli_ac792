...
static void indev_keypad_proc(lv_indev_t *i, lv_indev_data_t *data)
{
    ...
    /*Release happened*/
        ...
        if (data->key == LV_KEY_ENTER) {
            ...
            if (i->long_pr_sent == 0) {
                if (indev_proc_short_click(i) == LV_RESULT_INVALID) {
                    return;
                }
            }
...
static void indev_proc_press(lv_indev_t *indev)
{
    ...
    if (indev_obj_act) {
        ...
        for (int i = 0; i < LV_INDEV_GESTURE_CNT; i++) {
            ...
            if (indev->gesture_type[i] != LV_INDEV_GESTURE_NONE) {
                indev->cur_gesture = (lv_indev_gesture_type_t) i;
                if (send_event(LV_EVENT_GESTURE, indev_act) == LV_RESULT_INVALID) {
                    return;
                }
                break;
            }
            ...
            if (scroll_obj == NULL) {
                if (indev->pointer.pressed) {
                    if (indev->long_pr_sent == 0) {
                        if (indev_proc_short_click(indev) == LV_RESULT_INVALID) {
                            return;
                        }
                    }
                    if (send_event(LV_EVENT_CLICKED, indev_act) == LV_RESULT_INVALID) {
