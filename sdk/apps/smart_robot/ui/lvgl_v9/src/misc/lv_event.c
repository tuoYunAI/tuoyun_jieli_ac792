...
lv_result_t lv_event_send(lv_event_list_t *list, lv_event_t *e, bool preprocess)
{
    ...
    for (uint32_t i = 0; i < size && !e->deleted; i++) {
        ...
        if (event_is_marked_deleting(dsc)) {
            continue;
        }
    ...
    if (e->deleted) {
        cleanup_event_list_core(&back_array_head);
    } else {
...
static void cleanup_event_list_core(lv_array_t *array)
{
    ...
    for (uint32_t i = 0; i < size; i++) {
        ...
        if (event_is_marked_deleting(*dsc_i)) {
            lv_free(*dsc_i);
        } else {
            *dsc_kept = *dsc_i;
            kept_count++;
    ...
    if (kept_count == 0) {
        ...
        lv_array_resize(array, kept_count);
    }
}

static void cleanup_event_list(lv_event_list_t *list)
{
    if (list->is_traversing) {
        return;
    }
    if (list->has_marked_deleting == false) {
        return;
    }
